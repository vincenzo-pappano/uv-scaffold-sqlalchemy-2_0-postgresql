...
```
```
SET search_path TO mfr, public;
CREATE TABLE IF NOT EXISTS mfr.alembic_version (
...
--------+-----------------+-------+----------
 mfr    | alembic_version | table | fms_user
 mfr    | mfr_records     | table | fms_user
(2 rows)
```
If the **alembic_version** table already exists you will see:
```
NOTICE:  relation "alembic_version" already exists, skipping
```
</details>
...
* Keep schema-aware settings (search_path, version_table_schema="mfr").
* Commit both alembic/ and migrations into Git.
</details>